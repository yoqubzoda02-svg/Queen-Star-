...
import { doc, getDoc, getDocs, addDoc, collection, onSnapshot, query, where, setDoc } from 'firebase/firestore';
...
function CartPage({ cart, onClose, onCheckout, onRemove, customer, onNeedAuth }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:BG, zIndex:200, overflowY:'auto' }}>
      ...
      {cart.length>0 && (
        <div style={{ position:'fixed', bottom:0, left:0, right:0, background:PAPER, borderTop:`1px solid ${LINE}`, padding:16, boxShadow:'0 -8px 24px rgba(35,26,20,0.06)' }}>
...
function OrderForm({ cart, customer, discount, onClose, onSuccess }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:BG, zIndex:300, overflowY:'auto' }}>
      ...
      <div style={{ position:'fixed', bottom:0, left:0, right:0, padding:16, background:PAPER, borderTop:`1px solid ${LINE}` }}>
        <div style={{ ...S.page, padding:0 }}>
    ...
    <div style={{ background:BG, minHeight:'100vh', color:INK, fontFamily:"'Manrope',sans-serif" }}>
      ...
      {screen==='catalog' && (
        <div style={{ ...S.page, padding:'14px 14px 30px' }}>
          ...
          <div style={{ marginBottom:14 }}><StoreHours/></div>
          <FlashSaleBanner/>
