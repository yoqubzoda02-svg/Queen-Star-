...
import { doc, setDoc, onSnapshot, collection, addDoc, query, orderBy, getDoc, updateDoc } from 'firebase/firestore';
...
const INIT_SETTINGS={usdRate:10.9,cnyRate:1.5,cargoUsd:2.6,markup:3,pin:'1234',employeePin:'1111',lowStock:5,telegramToken:'',telegramChatId:'',monthlyGoal:15000};
...
// ── EMPLOYEE SIMPLE LOGIN ─────────────────────────────────────
function EmployeeSimpleLogin({onLogin, employeePin}){
  ...
  const submit=async()=>{
    ...
    if(pin!==employeePin){setErr('Неверный PIN сотрудника');return;}
    const empId=name.trim().toLowerCase().replace(/\s+/g,'_');
    try{await setDoc(doc(db,'employees',empId),{name:name.trim(),id:empId,lastLogin:nowStr()});}catch{}
    onLogin('employee',{name:name.trim(),id:empId});
  };

  return(
    <div style={{width:'100%',maxWidth:300}}>
      <div style={{fontSize:13,color:MUTED,textAlign:'center',marginBottom:16}}>Вход для сотрудника</div>
      <div style={{marginBottom:10}}>
        <div style={S.label}>Твоё имя</div>
        <input style={S.input} value={name} onChange={e=>{setName(e.target.value);setErr('');}} placeholder="Шахло, Малика..." autoFocus/>
      </div>
      <div style={{marginBottom:14}}>
        <div style={S.label}>PIN сотрудника</div>
        <input style={{...S.input,textAlign:'center',fontSize:22,letterSpacing:8}} type="password" maxLength={6}
          value={pin} onChange={e=>{setPin(e.target.value);setErr('');}}
          onKeyDown={e=>{if(e.key==='Enter')submit();}} placeholder="••••"/>
      </div>
      {err&&<div style={{fontSize:12,color:RED,textAlign:'center',marginBottom:10}}>{err}</div>}
      <button style={S.btn(GOLD)} onClick={submit}>Войти</button>
      <div style={{fontSize:11,color:MUTED,textAlign:'center',marginTop:10}}>PIN задаёт владелец в настройках</div>
    </div>
  );
}

function LoginScreen({onLogin,ownerPin,employeePin}){
  ...
  return(
    <div style={{background:BG,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:28}}>
      ...
      {mode==='employee'&&(
        <>
          <EmployeeSimpleLogin onLogin={onLogin} employeePin={employeePin||'1111'}/>
  ...
  return(
    <div>
      ...
      <div style={{...S.row,marginBottom:10}}>
        ...
        <StatBox label="Анна (35%)" value={fmt(annaShare)} unit="сом" color={BLUE}/>
      </div>
      {settings.monthlyGoal>0&&(
        <div style={{...S.card,marginBottom:14}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
            <div style={{fontSize:11,color:MUTED}}>ПЛАН МЕСЯЦА</div>
            <div style={{fontSize:11,color:GOLD,fontWeight:700}}>{fmt(monthRevenue)} / {fmt(settings.monthlyGoal)} сом</div>
          </div>
          <div style={{height:8,background:SURF,borderRadius:4,overflow:'hidden'}}>
            <div style={{height:'100%',width:`${Math.min(100,Math.round(monthRevenue/settings.monthlyGoal*100))}%`,background:`linear-gradient(90deg,${GOLD},${GREEN})`,borderRadius:4,transition:'width 0.5s'}}/>
          </div>
          <div style={{fontSize:10,color:MUTED,marginTop:4}}>{Math.min(100,Math.round(monthRevenue/settings.monthlyGoal*100))}% выполнено</div>
        </div>
      )}
      {(cash>0||trans>0)&&(
        <div style={S.card}>
...
function Kassa({products,addSale,deleteSale,todaySales,role}){
  ...
  const submit=()=>{
    ...
    addSale(prodId,qty,price,note,payment,size);
  ...
  return(
    <div>
      <div style={S.card}>
        ...
        <div style={{...S.row,marginBottom:10}}>
          <div style={S.half}><div style={S.label}>Размер (необяз.)</div><input style={S.input} value={size} onChange={e=>setSize(e.target.value)} placeholder="38"/></div>
      ...
      {todaySales.map(s=>(
        <div key={s.id} style={{...S.li,alignItems:'flex-start'}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:600}}>{s.name}</div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{fontWeight:700,color:GREEN}}>{fmt(s.total)}</div>
            <button onClick={()=>deleteSale(s.id)} style={{background:'none',border:`1px solid ${RED}`,color:RED,borderRadius:6,padding:'4px 8px',cursor:'pointer',fontSize:12}}>🗑</button>
          </div>
        </div>
      ))}
    </div>
...
function ProductCard({product,updateProduct,deleteProduct,transit,photos,onPhoto,settings}){
    ...
    <div style={{...S.card,borderColor:isLow?AMBER+'55':BORD,opacity:transit?0.72:1}}>
      ...
      {!transit&&!editing&&(
        <div>
          ...
          <button onClick={()=>{if(window.confirm('Удалить товар '+product.name+'?'))deleteProduct(product.id);}}
            style={{...S.btnSm(RED),width:'100%',marginTop:6,textAlign:'center'}}>🗑 Удалить товар</button>
...
function Stock({products,updateProduct,addProduct,deleteProduct,photos,onPhoto,role,settings}){
  ...
  return(
    <div>
      ...
      {inStock.map(p=><ProductCard key={p.id} product={p} updateProduct={updateProduct} deleteProduct={deleteProduct} photos={photos} onPhoto={onPhoto} role={role} settings={settings}/>)}
      {inTransit.length>0&&(
        <>
...
function Finance({expenses,addExpense,deleteExpense,monthRevenue,monthCOGS,monthExpenses,monthProfit,sales}){
  ...
  return(
    <div>
      ...
      {expenses.filter(e=>e.date.startsWith(new Date().toISOString().slice(0,7))).map(e=>(
        <div key={e.id} style={{...S.li,alignItems:'flex-start'}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:600}}>{e.category}</div>
            <div style={{fontSize:11,color:MUTED}}>{shortDate(e.date)}{e.note?' · '+e.note:''}</div>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{fontWeight:700,color:RED}}>− {fmt(e.amount)}</div>
            <button onClick={()=>deleteExpense(e.id)}
              style={{background:'none',border:'none',color:RED,fontSize:16,cursor:'pointer',padding:'0 4px'}}>🗑</button>
          </div>
        </div>
      ))}
    </div>
...
function History({sales,onBack,deleteSale}){
          ...
          <div key={day}>
            ...
            {byDay[day].map(s=>(
              <div key={s.id} style={{...S.li,alignItems:'flex-start'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  <div style={{fontWeight:700,color:GREEN}}>{fmt(s.total)}</div>
                  <button onClick={()=>deleteSale&&deleteSale(s.id)} style={{background:'none',border:'none',color:RED,fontSize:16,cursor:'pointer'}}>🗑</button>
                </div>
              </div>
            ))}
          </div>
...
function More({sales,expenses,products,monthRevenue,monthCOGS,monthExpenses,monthProfit,annaShare,settings,setSettings,onLogout,orders,setOrders,customers,addCustomer,purchases,addPurchase,updatePurchase,debts,addDebt,updateDebt,photos,returns,addReturn,shopOrders=[],updateShopOrder,deleteSale,deleteOrder,deleteCustomer,deletePurchase,deleteDebt,deleteReturn,currentEmployee}){
  const [empList,setEmpList]=useState([]);
  useEffect(()=>{try{const u=onSnapshot(collection(db,'employees'),s=>setEmpList(s.docs.map(d=>({id:d.id,...d.data()}))));return()=>u();}catch{}},[]);
  const [view,setView]=useState(null);

  // shifts view removed
  if(view==='calc1688') return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><PurchaseCalc1688 settings={settings}/></div>;
  if(view==='stores')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><StoresManager/></div>;
  if(view==='chart')    return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><SalesChart sales={sales}/></div>;
  if(view==='anna')     return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><AnnaReportView sales={sales} expenses={expenses} settings={settings}/></div>;
  if(view==='shift')    return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><ShiftManager sales={sales} currentEmployee={currentEmployee}/></div>;
  if(view==='telegram') return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><TelegramSetup settings={settings} setSettings={setSettings}/></div>;
  if(view==='returns')  return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Returns returns={returns} addReturn={addReturn} deleteReturn={deleteReturn} products={products}/></div>;
  if(view==='salary')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><SalaryCalc sales={sales} employees={empList} settings={settings}/></div>;
  if(view==='weekly')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><WeeklyReport sales={sales} expenses={expenses} orders={orders}/></div>;
  if(view==='hashtags') return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><HashtagGen/></div>;
  if(view==='content')  return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><ContentPlan/></div>;
  ...
  if(view==='orders')   return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Orders orders={orders} setOrders={setOrders} products={products} shopOrders={shopOrders} updateShopOrder={updateShopOrder} deleteOrder={deleteOrder}/></div>;
  if(view==='customers')return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Customers customers={customers} addCustomer={addCustomer} deleteCustomer={deleteCustomer} orders={orders}/></div>;
  ...
  if(view==='purchases')return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Purchases purchases={purchases} addPurchase={addPurchase} updatePurchase={updatePurchase} deletePurchase={deletePurchase}/></div>;
  if(view==='debts')    return <div><button style={{...S.btnSm(MUTED),marginBottom:12}} onClick={()=>setView(null)}>← Назад</button><Debts debts={debts} addDebt={addDebt} updateDebt={updateDebt} deleteDebt={deleteDebt}/></div>;
  if(view==='history')  return <History sales={sales} onBack={()=>setView(null)} deleteSale={deleteSale}/>;
    ...
    <div>
      ...
      <div style={{...S.card,borderColor:BLUE+'44'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
          ...
          <div><div style={{fontWeight:700,fontSize:15}}>Анна Порошина</div><div style={{fontSize:11,color:BLUE,marginTop:2}}>Инвестор · 35% прибыли · ₽100 000</div></div>
        </div>
        <FR label="Выручка" value={fmt(monthRevenue)+' сом'} color={GREEN}/>
        <FR label="Все расходы" value={`− ${fmt(monthCOGS+monthExpenses)} сом`} color={RED}/>
        <div style={S.div}/>
        <FR label="Чистая прибыль" value={fmt(monthProfit)+' сом'} color={monthProfit>=0?GREEN:RED} bold/>
        <div style={{background:BLUE+'11',border:`1px solid ${BLUE}33`,borderRadius:12,padding:16,marginTop:14,textAlign:'center'}}>
          <div style={{fontSize:9,color:BLUE,letterSpacing:3,marginBottom:6}}>К ВЫПЛАТЕ АННЕ (35%)</div>
  ...
  if(view==='settings')return(
    <div>
      ...
      <div style={S.card}>
        <div style={S.sec}>🎯 План продаж на месяц</div>
        <div style={S.label}>Цель выручки (сомони)</div>
        <input style={S.input} type="number" value={settings.monthlyGoal||15000} onChange={e=>setSettings({...settings,monthlyGoal:Number(e.target.value)})}/>
      </div>
      <div style={S.card}>
        <div style={S.sec}>PIN сотрудника</div>
        <div style={S.label}>Сотрудники входят с этим PIN</div>
        <input style={S.input} type="password" maxLength={6}
          defaultValue={settings.employeePin||'1111'}
          onBlur={e=>setSettings({...settings,employeePin:e.target.value||'1111'})}
          placeholder="••••"/>
        <div style={{fontSize:11,color:MUTED,marginTop:6}}>По умолчанию: 1111</div>
      </div>
      <div style={S.card}>
        <div style={S.sec}>Сменить PIN владельца</div>
  ...
  const items=[
    ...
    {id:'telegram', icon:'📱',label:'Telegram',     sub:'Уведомления о продажах и заказах'},
    {id:'returns',  icon:'🔄',label:'Возвраты',     sub:'Оформить возврат товара'},
    {id:'salary',   icon:'💳',label:'Зарплата',     sub:'Расчёт зарплаты сотрудников'},
    {id:'weekly',   icon:'📊',label:'Отчёт недели', sub:'Сравнение с прошлой неделей'},
    {id:'hashtags', icon:'#️⃣',label:'Хэштеги',      sub:'Хэштеги для Instagram'},
    {id:'content',  icon:'📅',label:'Контент-план', sub:'План постов на неделю'},
    {id:'calc1688', icon:'🧮',label:'Калькулятор 1688', sub:'Цена закупки → цена продажи'},
    {id:'stores',    icon:'🏬',label:'Точки продаж',    sub:'Если открываешь второй магазин'},
    {id:'chart',    icon:'📊',label:'График продаж', sub:'Продажи за 14 дней'},
    {id:'anna',     icon:'📈',label:'Отчёт Анне',   sub:'PDF отчёт для инвестора'},
    {id:'shift',    icon:'⏱',label:'Смена',         sub:'Открыть или закрыть смену'},
...
// ── ЗАКАЗЫ ────────────────────────────────────────────────────
function Orders({orders,setOrders,products,shopOrders=[],updateShopOrder,deleteOrder}){
        ...
        return(
          <div key={o.id} style={{...S.card,borderColor:st.c+'44'}}>
            ...
            <div style={{...S.row,marginTop:10}}>
              ...
              <button style={{...S.btnSm(MUTED),flex:1}} onClick={()=>deleteOrder(o.id)}>🗑</button>
            </div>
          </div>
        );
      })}
      {shopOrders.length>0&&(
        <div style={{...S.card,borderColor:BLUE+'55',marginBottom:12,overflow:'visible'}}>
          <div style={{...S.sec,color:BLUE,padding:'12px 14px'}}>🌐 С сайта ({shopOrders.filter(o=>o.status!=='done'&&o.status!=='cancelled').length} активных)</div>
          {shopOrders.filter(o=>o.status!=='cancelled').sort((a,b)=>new Date(b.date)-new Date(a.date)).map(o=>(
            <div key={o.id} style={{borderTop:`1px solid ${BORD}`,padding:'12px 14px'}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                <div>
                  <div style={{fontSize:13,fontWeight:700}}>{o.customerName}</div>
                  <div style={{fontSize:11,color:MUTED}}>{o.productName} · р.{o.size} · {o.qty}шт</div>
                  <div style={{fontSize:11,color:MUTED}}>📞 {o.customerPhone}</div>
                  {o.address&&<div style={{fontSize:11,color:BLUE}}>📍 {o.address}</div>}
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontWeight:700,color:GOLD}}>{fmt(o.total)} сом</div>
                  <Badge text={o.status==='new'?'Новый':o.status==='delivery'?'В пути':'Выдан'} color={o.status==='new'?GOLD:o.status==='delivery'?BLUE:GREEN}/>
                  <Badge text="С сайта" color={BLUE}/>
                </div>
              </div>
              {o.status!=='done'&&updateShopOrder&&(
                <div style={{...S.row,gap:6,marginTop:8}}>
                  {o.status==='new'&&<button style={{...S.btnSm(BLUE),flex:2,textAlign:'center'}} onClick={()=>updateShopOrder(o.id,'delivery')}>🚚 В пути</button>}
                  {(o.status==='new'||o.status==='delivery')&&<button style={{...S.btnSm(GREEN),flex:2,textAlign:'center'}} onClick={()=>updateShopOrder(o.id,'done')}>✓ Выдан</button>}
                  <button style={{...S.btnSm(RED),flex:1,textAlign:'center'}} onClick={()=>updateShopOrder(o.id,'cancelled')}>✕</button>
                </div>
              )}
            </div>
          ))}
...
// ── КЛИЕНТЫ ───────────────────────────────────────────────────
function Customers({customers,addCustomer,deleteCustomer,orders}){
        ...
        <div key={c.id} style={S.card}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            ...
            <div style={{textAlign:'right'}}>
              ...
              <button onClick={()=>deleteCustomer(c.id)} style={{background:'none',border:'none',color:RED,fontSize:14,cursor:'pointer',marginTop:4}}>🗑 Удалить</button>
            </div>
          </div>
        </div>
      ))}
...
// ── ЗАКУПКИ ───────────────────────────────────────────────────
function Purchases({purchases,addPurchase,updatePurchase,deletePurchase}){
  const [show,setShow]=useState(false);
      ...
      {active.map(p=>{
        ...
        return(
          <div key={p.id} style={{...S.card,borderColor:st.c+'44'}}>
            ...
            <div style={{...S.row,marginTop:10}}>
              {NEXT[p.status]&&(
                <button style={{...S.btnSm(GREEN),flex:3}} onClick={()=>updatePurchase(p.id,'status',NEXT[p.status])}>
                  {p.status==='planned'?'✓ Заказано':p.status==='ordered'?'🚚 Отправили':'📦 Получили'}
                </button>
              )}
              <button style={{...S.btnSm(RED),flex:1}} onClick={()=>deletePurchase(p.id)}>🗑</button>
            </div>
          </div>
        );
      })}
...
// ── ДОЛГИ ─────────────────────────────────────────────────────
function Debts({debts,addDebt,updateDebt,deleteDebt}){
  const [show,setShow]=useState(false);
    ...
    <div>
      ...
      {active.map(d=>(
        <div key={d.id} style={{...S.card,borderColor:(d.type==='debt'?GREEN:BLUE)+'44'}}>
          ...
          <div style={{...S.row,marginTop:10}}>
            <button style={{...S.btnSm(GREEN),flex:3}} onClick={()=>updateDebt(d.id,'paid',true)}>✓ Погашено</button>
            <button style={{...S.btnSm(RED),flex:1}} onClick={()=>deleteDebt(d.id)}>🗑</button>
          </div>
        </div>
      ))}
...
function Receipt({sale,onClose}){
  ...
  const lines=[
    ...
    '─────────────────────',
    sale.name,
    `${sale.qty} пар × ${fmt(sale.price)} сом`,
    sale.size?`Размер: ${sale.size}`:'',
    '─────────────────────',
    `ИТОГО: ${fmt(sale.total)} сом`,
    sale.paymentMethod==='cash'?'💵 Наличные':'📲 Перевод',
    new Date(sale.date).toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}),
    '─────────────────────',
  ...
  const copy=()=>{
    ...
    alert('Скопировано! Отправь клиенту в WhatsApp/Telegram');
  };
  const share=()=>{
    if(navigator.share) navigator.share({text:lines}).catch(()=>{});
    else copy();
  };
  const printBt=()=>printReceiptBluetooth(lines, setBtStatus);
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:CARD,borderRadius:16,padding:20,maxWidth:320,width:'100%',border:`1px solid ${GOLD}44`}}>
        <div style={{textAlign:'center',marginBottom:14}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:GOLD,letterSpacing:3}}>QUEEN STAR</div>
        </div>
        <div style={{background:SURF,borderRadius:8,padding:14,fontFamily:'monospace',fontSize:13,marginBottom:14,whiteSpace:'pre-wrap',color:CREAM,lineHeight:1.7}}>{lines}</div>
        <div style={{...S.row,gap:8,marginBottom:8}}>
          <button style={{...S.btn(GOLD),flex:2}} onClick={share}>📤 Отправить</button>
          <button style={{...S.btnSm(MUTED),padding:'12px 16px'}} onClick={onClose}>✕</button>
        </div>
        <button style={{...S.btnSm(BLUE),width:'100%',textAlign:'center'}} onClick={printBt}>🖨 Печать через Bluetooth</button>
        {btStatus&&<div style={{fontSize:11,color:MUTED,textAlign:'center',marginTop:6}}>{btStatus}</div>}
      </div>
    </div>
  );
}
...
function TelegramSetup({settings,setSettings}){
  const [msg,setMsg]=useState('');
  const test=async()=>{
    if(!settings.telegramToken||!settings.telegramChatId){setMsg('Заполни токен и Chat ID');return;}
    try{
      const r=await fetch(`https://api.telegram.org/bot${settings.telegramToken}/sendMessage`,{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({chat_id:settings.telegramChatId,text:'✅ Queen Star подключён!\nУведомления работают.'})
      });
      const d=await r.json();
      setMsg(d.ok?'✅ Сообщение отправлено!':'❌ Ошибка: '+d.description);
    }catch(e){setMsg('❌ Ошибка сети');}
  };
  return(
    <div style={S.card}>
      <div style={S.sec}>📱 Telegram уведомления</div>
      <div style={{fontSize:12,color:MUTED,marginBottom:12,lineHeight:1.7}}>
        Новый заказ или продажа → сразу сообщение тебе в Telegram.<br/>
        Работает в России без VPN.
      </div>
      <div style={{...S.card,background:SURF,marginBottom:0}}>
        <div style={{fontSize:11,fontWeight:700,color:GOLD,marginBottom:8}}>КАК НАСТРОИТЬ:</div>
        <div style={{fontSize:11,color:MUTED,lineHeight:1.8}}>
          1. Напиши @BotFather в Telegram<br/>
          2. Отправь /newbot → введи имя<br/>
          3. Скопируй токен (выглядит как 123456:ABC...)<br/>
          4. Открой бот и нажми Start<br/>
          5. Зайди на: api.telegram.org/bot<b style={{color:CREAM}}>ТВОЙ_ТОКЕН</b>/getUpdates<br/>
          6. Найди "id" в "chat" — это твой Chat ID
        </div>
      </div>
      <div style={{marginTop:12,marginBottom:8}}>
        <div style={S.label}>Токен бота</div>
        <input style={S.input} value={settings.telegramToken} onChange={e=>setSettings({...settings,telegramToken:e.target.value})} placeholder="123456789:AABBcc..."/>
      </div>
      <div style={{marginBottom:12}}>
        <div style={S.label}>Chat ID</div>
        <input style={S.input} value={settings.telegramChatId} onChange={e=>setSettings({...settings,telegramChatId:e.target.value})} placeholder="123456789"/>
      </div>
      {msg&&<div style={{fontSize:12,color:msg.startsWith('✅')?GREEN:RED,marginBottom:8}}>{msg}</div>}
      <button style={S.btn(GOLD)} onClick={test}>📨 Тест отправки</button>
    </div>
  );
}

// ── ВОЗВРАТЫ ─────────────────────────────────────────────────
function Returns({returns,addReturn,deleteReturn,products}){
  const [show,setShow]=useState(false);
  const [f,setF]=useState({productId:'',qty:'1',reason:'',size:''});
  const [msg,setMsg]=useState('');
  const inStock=products.filter(p=>!p.transit);
  const submit=()=>{
    if(!f.productId||!f.qty){setMsg('Выбери товар');return;}
    const p=products.find(x=>x.id===f.productId);
    addReturn({...f,productName:p?.name||'',qty:Number(f.qty)});
    setF({productId:'',qty:'1',reason:'',size:''});
    setShow(false);setMsg('✓ Возврат оформлен');setTimeout(()=>setMsg(''),2000);
  };
  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <div style={S.sec}>Возвраты ({totalReturns})</div>
        <button style={S.btnSm(show?RED:GOLD)} onClick={()=>setShow(!show)}>{show?'✕':'+ Возврат'}</button>
      </div>
      {msg&&<div style={{fontSize:12,color:msg.startsWith('✓')?GREEN:RED,marginBottom:8}}>{msg}</div>}
      {show&&(
        <div style={{...S.card,borderColor:RED+'44',marginBottom:12}}>
          <div style={S.sec}>Оформить возврат</div>
          <div style={{marginBottom:8}}>
            <div style={S.label}>Товар</div>
            <select style={S.select} value={f.productId} onChange={e=>setF(v=>({...v,productId:e.target.value}))}>
              <option value="">— Выберите —</option>
              {inStock.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div style={{...S.row,marginBottom:8}}>
            <div style={S.half}><div style={S.label}>Кол-во</div><input style={S.input} type="number" min="1" value={f.qty} onChange={e=>setF(v=>({...v,qty:e.target.value}))}/></div>
            <div style={S.half}><div style={S.label}>Размер</div><input style={S.input} value={f.size} onChange={e=>setF(v=>({...v,size:e.target.value}))} placeholder="38"/></div>
          </div>
          <div style={{marginBottom:12}}><div style={S.label}>Причина</div><input style={S.input} value={f.reason} onChange={e=>setF(v=>({...v,reason:e.target.value}))} placeholder="Не подошёл размер..."/></div>
          <button style={S.btn(RED)} onClick={submit}>Оформить возврат</button>
        </div>
      )}
      {monthReturns.length===0&&<div style={{color:MUTED,textAlign:'center',padding:'20px 0',fontSize:13}}>Возвратов нет</div>}
      {monthReturns.map(r=>(
        <div key={r.id} style={{...S.li,alignItems:'flex-start'}}>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:600}}>{r.productName}</div>
            <div style={{fontSize:11,color:MUTED}}>{r.qty} пар{r.size?' · р.'+r.size:''} · {shortDate(r.date)}</div>
            {r.reason&&<div style={{fontSize:11,color:RED}}>{r.reason}</div>}
          </div>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <Badge text="Возврат" color={RED}/>
            <button onClick={()=>deleteReturn(r.id)} style={{background:'none',border:'none',color:RED,fontSize:16,cursor:'pointer'}}>🗑</button>
          </div>
        </div>
      ))}
    </div>
...
// ── ЗАРПЛАТА ─────────────────────────────────────────────────
function SalaryCalc({sales,employees=[],settings}){
  const [salaries,setSalaries]=useState({});
  const thisMonth=new Date().toISOString().slice(0,7);
  const monthSales=sales.filter(s=>s.date.startsWith(thisMonth));

  const calcSalary=(emp)=>{
    const fixed=Number(salaries[emp.id]?.fixed||2200);
    const pct=Number(salaries[emp.id]?.pct||0);
    const empSales=monthSales.filter(s=>s.employeeId===emp.id||s.employeeName===emp.name);
    const empRevenue=empSales.reduce((s,x)=>s+x.total,0);
    const bonus=Math.round(empRevenue*pct/100);
    return{fixed,pct,empRevenue,bonus,total:fixed+bonus,count:empSales.length};
  };

  const FIXED_COURIER=3000;
  return(
    <div>
      <div style={S.sec}>Расчёт зарплат</div>
      <div style={{...S.card,borderColor:AMBER+'44',marginBottom:12}}>
        <FR label="Курьер (фикс)" value={fmt(FIXED_COURIER)+' сом'} color={AMBER}/>
        <div style={{fontSize:11,color:MUTED,marginTop:6}}>Месяц: {new Date().toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}</div>
      </div>
      {employees.length===0&&<div style={{color:MUTED,textAlign:'center',padding:'20px 0',fontSize:13}}>Нет сотрудников с SMS входом</div>}
      {employees.map(emp=>{
        const calc=calcSalary(emp);
        return(
          <div key={emp.id} style={S.card}>
            <div style={{fontSize:15,fontWeight:700,marginBottom:10}}>{emp.name}</div>
            <div style={{...S.row,gap:6,marginBottom:10}}>
              <div style={S.half}>
                <div style={S.label}>Фикс (сом)</div>
                <input style={{...S.input,padding:'8px'}} type="number" value={salaries[emp.id]?.fixed||2200}
                  onChange={e=>setSalaries(p=>({...p,[emp.id]:{...p[emp.id],fixed:e.target.value}}))}/>
              </div>
              <div style={S.half}>
                <div style={S.label}>Бонус % от продаж</div>
                <input style={{...S.input,padding:'8px'}} type="number" value={salaries[emp.id]?.pct||0}
                  onChange={e=>setSalaries(p=>({...p,[emp.id]:{...p[emp.id],pct:e.target.value}}))}/>
              </div>
            </div>
            <FR label="Продаж за месяц" value={calc.count+' шт'}/>
            <FR label="Выручка" value={fmt(calc.empRevenue)+' сом'} color={GREEN}/>
            <FR label={`Бонус (${calc.pct}%)`} value={fmt(calc.bonus)+' сом'} color={GOLD}/>
            <div style={S.div}/>
            <FR label="К выплате" value={fmt(calc.total)+' сом'} color={GREEN} bold/>
          </div>
        );
      })}
    </div>
...
// ── ОТЧЁТ НЕДЕЛИ ─────────────────────────────────────────────
function WeeklyReport({sales,expenses,orders}){
  const getWeekSales=(weeksAgo)=>{
    const now=new Date();
  ...
  const thisWeek=getWeekSales(0);
  const lastWeek=getWeekSales(1);
  const rev=thisWeek.reduce((s,x)=>s+x.total,0);
  const lastRev=lastWeek.reduce((s,x)=>s+x.total,0);
  const diff=rev-lastRev;
  const diffPct=lastRev>0?Math.round((diff/lastRev)*100):0;
  const avgCheck=thisWeek.length>0?Math.round(rev/thisWeek.length):0;
  const byProd=thisWeek.reduce((acc,s)=>{acc[s.name]=(acc[s.name]||0)+s.total;return acc;},{});
  const topProd=Object.entries(byProd).sort((a,b)=>b[1]-a[1])[0];
  const doneOrders=orders.filter(o=>o.status==='done'&&new Date(o.date)>=new Date(new Date().setDate(new Date().getDate()-7))).length;

  return(
    <div>
      <div style={S.sec}>📊 Отчёт этой недели</div>
      <div style={{...S.row,marginBottom:10}}>
        <StatBox label="Выручка" value={fmt(rev)} unit="сом" color={GREEN}/>
        <StatBox label="Продаж" value={thisWeek.length} unit="шт" color={GOLD}/>
      </div>
      <div style={{...S.row,marginBottom:14}}>
        <StatBox label="Средний чек" value={fmt(avgCheck)} unit="сом" color={BLUE}/>
        <StatBox label="Заказов выдано" value={doneOrders} unit="шт" color={AMBER}/>
      </div>
      <div style={S.card}>
        <div style={S.sec}>Сравнение с прошлой неделей</div>
        <FR label="Прошлая неделя" value={fmt(lastRev)+' сом'}/>
        <FR label="Эта неделя" value={fmt(rev)+' сом'} color={GREEN}/>
        <div style={S.div}/>
        <FR label="Изменение" value={`${diff>=0?'+':''}${fmt(diff)} сом (${diffPct}%)`} color={diff>=0?GREEN:RED} bold/>
      </div>
      {topProd&&(
        <div style={{...S.card,borderColor:GOLD+'44'}}>
          <div style={S.sec}>🏆 Топ товар недели</div>
          <div style={{fontSize:16,fontWeight:700,color:GOLD,marginBottom:4}}>{topProd[0]}</div>
          <div style={{fontSize:13,color:MUTED}}>Продажи: {fmt(topProd[1])} сом</div>
        </div>
      )}
    </div>
  );
...
function HashtagGen(){
  ...
  const CATS={
    'Туфли':    '#обувьдушанбе #туфли #туфлидушанбе #обувьтаджикистан #женскаяобувь #queenstar #мода #стиль #туфлитаджикистан #обувьонлайн',
    'Босоножки': '#босоножки #босоножкидушанбе #летняяобувь #обувьдушанбе #queenstar #стиль #мода #босоножкитаджикистан #летомода #женскаяобувь',
    'Сапоги':   '#сапоги #сапогидушанбе #зимняяобувь #обувьтаджикистан #queenstar #осеньзима #мода #сапогитаджикистан #обувьонлайн #стиль',
    'Кроссовки':'#кроссовки #кроссовкидушанбе #спортобувь #queenstar #мода #стиль #кроссовкитаджикистан #обувьдушанбе #удобнаяобувь #спорт',
    'Балетки':  '#балетки #балеткидушанбе #удобнаяобувь #queenstar #женскаяобувь #мода #балеткитаджикистан #обувьонлайн #стиль #элегантность',
    'Тапочки':  '#тапочки #домашняяобувь #queenstar #душанбе #комфорт #домашниетапочки #обувьтаджикистан #удобно #мода',
  };
  const gen=()=>{if(cat)setTags(CATS[cat]||'');};
  const copy=()=>{navigator.clipboard.writeText(tags).catch(()=>{});alert('Скопировано!')};
  return(
    <div>
      <div style={S.sec}>🏷️ Хэштеги для Instagram</div>
      <div style={S.card}>
        <div style={{marginBottom:10}}>
          <div style={S.label}>Категория товара</div>
          <select style={S.select} value={cat} onChange={e=>setCat(e.target.value)}>
            <option value="">— Выберите —</option>
            {Object.keys(CATS).map(c=><option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <button style={{...S.btn(GOLD),marginBottom:10}} onClick={gen}>Сгенерировать</button>
        {tags&&(
          <>
            <div style={{background:SURF,borderRadius:8,padding:12,fontSize:12,color:CREAM,lineHeight:1.8,marginBottom:10,wordBreak:'break-word'}}>{tags}</div>
            <button style={S.btn(GREEN)} onClick={copy}>📋 Копировать хэштеги</button>
          </>
        )}
      </div>
    </div>
...
function ContentPlan(){
  const DAYS=['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];
  const TEMPLATES=[
    {icon:'👟',type:'Товар дня',desc:'Фото товара + цена + размеры'},
    {icon:'🎯',type:'Акция',desc:'Скидка или специальное предложение'},
    {icon:'📦',type:'Новинка',desc:'Новое поступление на склад'},
    {icon:'⭐',type:'Отзыв',desc:'Отзыв довольного покупателя'},
    {icon:'📸',type:'Stories',desc:'За кулисами магазина'},
    {icon:'💡',type:'Полезное',desc:'Как выбрать размер обуви'},
    {icon:'🎁',type:'Конкурс',desc:'Розыгрыш или акция для подписчиков'},
  ];
  const schedule=[
    {day:'Пн',template:TEMPLATES[0]},
    {day:'Вт',template:TEMPLATES[4]},
    {day:'Ср',template:TEMPLATES[1]},
    {day:'Чт',template:TEMPLATES[2]},
    {day:'Пт',template:TEMPLATES[0]},
    {day:'Сб',template:TEMPLATES[3]},
    {day:'Вс',template:TEMPLATES[6]},
  ];
  return(
    <div>
      <div style={S.sec}>📅 Контент-план</div>
      <div style={{...S.card,marginBottom:12}}>
        <div style={{fontSize:12,color:MUTED}}>Рекомендуемый план постов на неделю для @queenstar.shop</div>
      </div>
      {schedule.map((item,i)=>(
        <div key={i} style={{...S.card,display:'flex',gap:12,alignItems:'flex-start'}}>
          <div style={{width:40,height:40,borderRadius:10,background:GOLD+'22',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{item.template.icon}</div>
          <div style={{flex:1}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
              <div style={{fontSize:13,fontWeight:700}}>{item.day}</div>
              <Badge text={item.template.type} color={GOLD}/>
            </div>
            <div style={{fontSize:12,color:MUTED}}>{item.template.desc}</div>
          </div>
        </div>
      ))}
      <div style={{...S.card,borderColor:GOLD+'44'}}>
        <div style={{fontSize:11,color:MUTED,lineHeight:1.8}}>
          💡 Оптимальное время для постов в Таджикистане:<br/>
          🕙 10:00 — Утро (люди едут на работу)<br/>
          🕐 13:00 — Обед (перерыв)<br/>
          🕗 19:00 — Вечер (после работы) ← лучшее время
        </div>
      </div>
    </div>
  );
...
function SalesChart({sales=[]}){
  ...
  for(let i=days-1;i>=0;i--){
    const d=new Date();d.setDate(d.getDate()-i);
    const key=d.toISOString().slice(0,10);
    const total=sales.filter(s=>s.date.startsWith(key)).reduce((s,x)=>s+x.total,0);
    data.push({label:d.toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'}),total});
  }
  const max=Math.max(...data.map(d=>d.total),1);
  const W=300,H=120,PL=8,PR=8,PT=10,PB=30;
  const barW=(W-PL-PR)/days-2;

  return(
    <div style={{...S.card,padding:14,marginBottom:14}}>
      <div style={{...S.sec,marginBottom:8}}>📊 Продажи за 14 дней</div>
      <svg width="100%" viewBox={`0 0 ${W} ${H+PB}`} style={{display:'block'}}>
        {data.map((d,i)=>{
          const x=PL+i*(barW+2);
          const bh=Math.round((d.total/max)*(H-PT));
          const y=H-bh;
          return(
            <g key={i}>
              <rect x={x} y={y} width={barW} height={bh} rx={2}
                fill={d.total>0?GOLD:SURF} opacity={0.85}/>
              {i%3===0&&<text x={x+barW/2} y={H+PB-4} textAnchor="middle"
                fontSize={7} fill={MUTED}>{d.label}</text>}
              {d.total>0&&bh>16&&<text x={x+barW/2} y={y+10} textAnchor="middle"
                fontSize={7} fill="#000" fontWeight="bold">{fmt(d.total)}</text>}
            </g>
          );
        })}
      </svg>
      <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:MUTED,marginTop:4}}>
        <span>Максимум: {fmt(max)} сом</span>
        <span>Итого: {fmt(data.reduce((s,d)=>s+d.total,0))} сом</span>
      </div>
    </div>
  );
}
...
// ── ОТЧЁТ ДЛЯ АННЫ ───────────────────────────────────────────
function AnnaReportView({sales,expenses,settings}){
  const [month,setMonth]=useState(new Date().toISOString().slice(0,7));
  ...
  const generate=()=>{
    ...
    const cogs=mSales.reduce((s,x)=>s+(x.cost||0)*x.qty,0);
    const exp=mExp.reduce((s,x)=>s+x.amount,0);
    const profit=revenue-cogs-exp;
    const anna=Math.round(Math.max(0,profit)*0.35);
    const dt=new Date(month+'-01').toLocaleDateString('ru-RU',{month:'long',year:'numeric'});

    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"/>
    <style>body{font-family:serif;max-width:600px;margin:40px auto;color:#1A1210;}
    h1{color:#C9952A;letter-spacing:3px;font-size:24px;margin-bottom:4px;}
    .sub{color:#8A7A6A;font-size:12px;margin-bottom:30px;}
    table{width:100%;border-collapse:collapse;margin-bottom:20px;}
    td{padding:10px 0;border-bottom:1px solid #E8DDD0;font-size:15px;}
    td:last-child{text-align:right;font-weight:600;}
    .total{font-size:18px;font-weight:700;color:#C9952A;}
    .anna{background:#C9952A;color:#fff;padding:16px;border-radius:10px;text-align:center;font-size:20px;font-weight:700;margin-top:20px;}
    .footer{color:#8A7A6A;font-size:11px;text-align:center;margin-top:30px;}
    @media print{button{display:none}}
    </style></head><body>
    <button onclick="window.print()" style="padding:10px 20px;background:#C9952A;color:#fff;border:none;border-radius:8px;cursor:pointer;margin-bottom:20px;font-size:14px">🖨️ Распечатать / Скачать PDF</button>
    <h1>QUEEN STAR</h1>
    <div class="sub">Финансовый отчёт · ${dt}</div>
    <table>
      <tr><td>Выручка</td><td>${fmt(revenue)} сом</td></tr>
      <tr><td>Себестоимость</td><td>− ${fmt(cogs)} сом</td></tr>
      <tr><td>Расходы</td><td>− ${fmt(exp)} сом</td></tr>
      <tr><td class="total">Чистая прибыль</td><td class="total">${fmt(profit)} сом</td></tr>
    </table>
    <div class="anna">💰 Доля Анны (35%): ${fmt(anna)} сом</div>
    <div style="margin-top:16px;font-size:12px;color:#8A7A6A">
      Продаж: ${mSales.length} шт · Расходов: ${mExp.length} записей
    </div>
    <div class="footer">Сформировано ${new Date().toLocaleDateString('ru-RU')} · Queen Star</div>
    </body></html>`;
  ...
  return(
    <div>
      <div style={{...S.card,marginBottom:12}}>
        <div style={{...S.sec}}>📈 Отчёт для инвестора</div>
        <div style={{marginBottom:12}}>
          <div style={S.label}>Месяц</div>
          <input style={S.input} type="month" value={month} onChange={e=>setMonth(e.target.value)}/>
        </div>
        <button style={S.btn(GOLD)} onClick={generate}>📄 Сформировать PDF отчёт</button>
        <div style={{fontSize:11,color:MUTED,marginTop:8}}>
          Откроется страница для печати или сохранения в PDF
        </div>
      </div>
    </div>
  );
...
// ── СМЕНА (ОТКРЫТЬ/ЗАКРЫТЬ) ──────────────────────────────────
function ShiftManager({sales,currentEmployee}){
  const [shift,setShift]=useState(null);
  const [loading,setLoading]=useState(true);

  useEffect(()=>{
    try{
      getDoc(doc(db,'qs','current_shift')).then(snap=>{
        if(snap.exists()&&snap.data()?.v){
          setShift(JSON.parse(snap.data().v));
        }
        setLoading(false);
      });
  ...
  const saveShift=(data)=>{
    setShift(data);
    setDoc(doc(db,'qs','current_shift'),{v:JSON.stringify(data),ts:Date.now()}).catch(()=>{});
  };

  const open=()=>{
    const s={
      employeeName:(currentEmployee&&currentEmployee.name)||'Сотрудник',
      startTime:new Date().toISOString(),
      startSalesCount:sales.length,
      startRevenue:sales.filter(x=>x.date.startsWith(todayStr())).reduce((s,x)=>s+x.total,0)
    };
    saveShift(s);
  };
  ...
  const close=()=>{
    if(!shift)return;
    const todaySales=sales.filter(x=>x.date.startsWith(todayStr()));
    const shiftSales=todaySales.filter(x=>new Date(x.date)>=new Date(shift.startTime));
    const revenue=shiftSales.reduce((s,x)=>s+x.total,0);
    const count=shiftSales.length;
    const duration=Math.round((Date.now()-new Date(shift.startTime))/1000/60);

    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"/>
    <style>body{font-family:sans-serif;max-width:400px;margin:30px auto;color:#1A1210;}
    h2{color:#C9952A;}table{width:100%;border-collapse:collapse;}
    td{padding:8px 0;border-bottom:1px solid #eee;font-size:14px;}
    td:last-child{text-align:right;font-weight:600;}
    @media print{button{display:none}}</style></head><body>
    <button onclick="window.print()" style="padding:8px 16px;background:#C9952A;color:#fff;border:none;border-radius:6px;cursor:pointer;margin-bottom:16px">🖨️ Печать</button>
    <h2>QUEEN STAR — Отчёт смены</h2>
    <table>
      <tr><td>Сотрудник</td><td>${shift.employeeName}</td></tr>
      <tr><td>Начало</td><td>${new Date(shift.startTime).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</td></tr>
      <tr><td>Конец</td><td>${new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</td></tr>
      <tr><td>Длительность</td><td>${duration} мин</td></tr>
      <tr><td>Продаж</td><td>${count} шт</td></tr>
      <tr><td style="font-size:16px;font-weight:700;color:#C9952A">Выручка</td><td style="font-size:16px;font-weight:700;color:#C9952A">${fmt(revenue)} сом</td></tr>
    </table></body></html>`;
  ...
  return(
    <div style={S.card}>
      <div style={S.sec}>⏱ Управление сменой</div>
      {!shift?(
        <div>
          <div style={{fontSize:13,color:MUTED,marginBottom:12}}>Смена не открыта</div>
          <button style={S.btn(GREEN)} onClick={open}>▶ Открыть смену</button>
        </div>
      ):(
        <div>
          <div style={{fontSize:13,color:GREEN,marginBottom:4}}>● Смена открыта</div>
          <div style={{fontSize:12,color:MUTED,marginBottom:4}}>Сотрудник: {shift.employeeName}</div>
          <div style={{fontSize:12,color:MUTED,marginBottom:12}}>
            С {new Date(shift.startTime).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}
          </div>
          <button style={S.btn(RED)} onClick={close}>■ Закрыть смену + Отчёт</button>
        </div>
      )}
    </div>
  );
...
// ── АВТО-УВЕДОМЛЕНИЯ ─────────────────────────────────────────
// (Вызывается внутри QueenStar через useEffect)


// ── КАЛЬКУЛЯТОР ЗАКУПКИ 1688 ─────────────────────────────────
function PurchaseCalc1688({settings}){
  const [priceCny,setPriceCny]=useState('');
  const [weightKg,setWeightKg]=useState('0.8');
  const [qty,setQty]=useState('1');
  const [markup,setMarkup]=useState(String(settings.markup||3));

  const cny=Number(priceCny)||0;
  const w=Number(weightKg)||0;
  const n=Number(qty)||1;
  const mk=Number(markup)||1;
  const cnyRate=Number(settings.cnyRate)||1.5;
  const cargoUsd=Number(settings.cargoUsd)||2.6;
  const usdRate=Number(settings.usdRate)||10.9;

  const itemCostSom=cny*cnyRate;
  const cargoCostSom=w*cargoUsd*usdRate;
  const totalCostPerItem=itemCostSom+cargoCostSom;
  const totalCostAll=totalCostPerItem*n;
  const sellPricePerItem=Math.round(totalCostPerItem*mk/10)*10;
  const profitPerItem=sellPricePerItem-totalCostPerItem;
  const profitAll=profitPerItem*n;

  return(
    <div>
      <div style={S.sec}>🧮 Калькулятор закупки 1688</div>
      <div style={S.card}>
        <div style={{marginBottom:10}}>
          <div style={S.label}>Цена за штуку (юань ¥)</div>
          <input style={S.input} type="number" value={priceCny} onChange={e=>setPriceCny(e.target.value)} placeholder="25"/>
        </div>
        <div style={{...S.row,marginBottom:10}}>
          <div style={S.half}>
            <div style={S.label}>Вес 1 шт (кг)</div>
            <input style={S.input} type="number" step="0.1" value={weightKg} onChange={e=>setWeightKg(e.target.value)}/>
          </div>
          <div style={S.half}>
            <div style={S.label}>Количество</div>
            <input style={S.input} type="number" value={qty} onChange={e=>setQty(e.target.value)}/>
          </div>
        </div>
        <div style={{marginBottom:4}}>
          <div style={S.label}>Наценка (× раз)</div>
          <input style={S.input} type="number" step="0.1" value={markup} onChange={e=>setMarkup(e.target.value)}/>
        </div>
      </div>

      <div style={{...S.card,borderColor:GOLD+'44'}}>
        <div style={S.sec}>Результат на 1 пару</div>
        <FR label="Цена товара" value={fmt(itemCostSom)+' сом'}/>
        <FR label="Карго (доставка)" value={fmt(cargoCostSom)+' сом'}/>
        <div style={S.div}/>
        <FR label="Себестоимость" value={fmt(totalCostPerItem)+' сом'} color={AMBER} bold/>
        <FR label="Цена продажи" value={fmt(sellPricePerItem)+' сом'} color={GOLD} bold/>
        <FR label="Прибыль с пары" value={fmt(profitPerItem)+' сом'} color={GREEN} bold/>
      </div>

      {n>1&&(
        <div style={{...S.card,borderColor:GREEN+'44'}}>
          <div style={S.sec}>Итого на {n} шт</div>
          <FR label="Общая себестоимость" value={fmt(totalCostAll)+' сом'} color={AMBER}/>
          <FR label="Общая прибыль" value={fmt(profitAll)+' сом'} color={GREEN} bold/>
        </div>
      )}

      <div style={{fontSize:11,color:MUTED,textAlign:'center',marginTop:6}}>
        Расчёт использует курсы из Настроек (CNY, USD, карго)
      </div>
    </div>
  );
}

// ── BLUETOOTH ПЕЧАТЬ ЧЕКА ────────────────────────────────────
async function printReceiptBluetooth(lines, onStatus){
  if(!navigator.bluetooth){
    onStatus('❌ Bluetooth не поддерживается в этом браузере');
    return;
  }
  try{
    onStatus('🔍 Поиск принтера...');
    ...
    if(!writeChar){ onStatus('❌ Не найден канал печати у устройства'); return; }
    onStatus('🖨 Печать...');
    ...
    onStatus('✅ Чек отправлен на печать!');
  }catch(e){
    console.error('Bluetooth print error:',e);
    onStatus('❌ Не удалось напечатать. Проверь что принтер включён и рядом.');
  }
}
...
function StoresManager(){
  ...
  useEffect(()=>{
    getDoc(doc(db,'qs','stores')).then(snap=>{
      if(snap.exists()&&snap.data()?.v){
        try{ setStores(JSON.parse(snap.data().v)); }catch{}
      }
      setLoading(false);
    }).catch(()=>setLoading(false));
  },[]);
  ...
  const save=(next)=>{
    setStores(next);
    setDoc(doc(db,'qs','stores'),{v:JSON.stringify(next),ts:Date.now()}).catch(()=>{});
  };

  const add=()=>{
    if(!f.name.trim())return;
    save([...stores,{id:Date.now(),...f}]);
    setF({name:'',address:'',phone:''});
    setShow(false);
  };

  const remove=(id)=>save(stores.filter(s=>s.id!==id));

  if(loading) return <div style={{color:MUTED,textAlign:'center',padding:24}}>Загрузка...</div>;

  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <div style={S.sec}>🏬 Точки продаж ({stores.length})</div>
        <button style={S.btnSm(show?RED:GOLD)} onClick={()=>setShow(!show)}>{show?'✕':'+ Добавить'}</button>
      </div>

      {show&&(
        <div style={{...S.card,borderColor:GOLD+'44',marginBottom:12}}>
          <div style={{marginBottom:8}}>
            <div style={S.label}>Название точки</div>
            <input style={S.input} value={f.name} onChange={e=>setF(v=>({...v,name:e.target.value}))} placeholder="Queen Star — Сомони"/>
          </div>
          <div style={{marginBottom:8}}>
            <div style={S.label}>Адрес</div>
            <input style={S.input} value={f.address} onChange={e=>setF(v=>({...v,address:e.target.value}))} placeholder="ул. Рудаки, 25"/>
          </div>
          <div style={{marginBottom:12}}>
            <div style={S.label}>Телефон</div>
            <input style={S.input} value={f.phone} onChange={e=>setF(v=>({...v,phone:e.target.value}))} placeholder="+992..."/>
          </div>
          <button style={S.btn(GOLD)} onClick={add}>Добавить точку</button>
        </div>
      )}

      {stores.length===0&&!show&&(
        <div style={{color:MUTED,textAlign:'center',padding:'24px 0',fontSize:13}}>
          Точек продаж пока нет.<br/>Добавь, если открываешь второй магазин.
        </div>
      )}
        ...
        <div key={s.id} style={S.card}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              ...
              {s.address&&<div style={{fontSize:12,color:MUTED,marginTop:2}}>📍 {s.address}</div>}
              {s.phone&&<div style={{fontSize:12,color:MUTED,marginTop:2}}>📞 {s.phone}</div>}
            </div>
            <button onClick={()=>remove(s.id)} style={{background:'none',border:'none',color:RED,fontSize:16,cursor:'pointer'}}>🗑</button>
          </div>
        </div>
      ))}
    </div>
...
export default function QueenStar(){
  ...
  const fsSettings  = useRef(null);
  const settingsRef = useRef(INIT_SETTINGS);
  useEffect(()=>{ settingsRef.current = settings; }, [settings]);

  // ── SIMPLE RELIABLE SYNC ──────────────────────────────────
  // Pre-fill saving.current with initial values
  // This prevents empty default data from overwriting Firebase when network is slow
  const saving = useRef({
    products:  JSON.stringify(INIT_PRODUCTS),
    sales:     JSON.stringify([]),
    expenses:  JSON.stringify([]),
    settings:  JSON.stringify(INIT_SETTINGS),
  ...
  const saveFS = (key, data) => {
    const v = JSON.stringify(data);
    if (saving.current[key] === v) return;
    const ts = Date.now();
    saving.current[key] = v;
    saving.current[key+'_ts'] = ts;
    saveQueue.current[key] = { v, ts };
    setSaveStatus('saving');
    setDoc(doc(db, 'qs', key), { v, ts })
      .then(() => {
        delete saveQueue.current[key];
        if (Object.keys(saveQueue.current).length === 0) setSaveStatus('ok');
      })
      .catch(() => setSaveStatus('error'));
  };

  // Retry on internet return or every 30s
  useEffect(() => {
    const retry = () => {
      const pending = Object.entries(saveQueue.current);
      if (!pending.length) return;
      setSaveStatus('saving');
      pending.forEach(([key, {v, ts}]) =>
        setDoc(doc(db, 'qs', key), {v, ts})
          .then(() => { delete saveQueue.current[key]; if(!Object.keys(saveQueue.current).length) setSaveStatus('ok'); })
          .catch(() => setSaveStatus('error'))
      );
    };
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const keys = ['products','sales','expenses','settings','photos','orders','customers','purchases','debts','returns'];
        const sets = [setProducts,setSales,setExpenses,setSettings,setPhotos,setOrders,setCustomers,setPurchases,setDebts,setReturns];
        const snaps = await Promise.allSettled(keys.map(k => getDoc(doc(db,'qs',k))));
        snaps.forEach((s,i) => {
          if (s.status==='fulfilled' && s.value?.exists && s.value.exists()) {
            try {
              const v = s.value.data()?.v;
              if (v) {
                const parsed = JSON.parse(v);
                sets[i](parsed);
                saving.current[keys[i]] = v; // mark as loaded
                if (keys[i]==='settings') fsSettings.current = parsed;
              }
            } catch {}
          }
        });
    ...
    const timer = setTimeout(() => setReady(true), 10000); // 10s fallback
    loadAll().finally(() => clearTimeout(timer));
  }, []);
      ...
      try {
        return onSnapshot(
          ...
          snap => {
            ...
            const v = snap.data()?.v;
            if (!v) return;
            // Only update if data is different from what we last saved
            if (v === saving.current[key]) return; // same as what we saved
            const inTs = snap.data().ts || 0;
            const myTs = saving.current[key+'_ts'] || 0;
            if (inTs < myTs) return; // older than our last save - ignore
            try {
              setter(JSON.parse(v));
              saving.current[key] = v;
              saving.current[key+'_ts'] = inTs;
            } catch {}
          },
          err => console.warn('Listen err:', key, err.code)
        );
      } catch { return () => {}; }
    };
    const unsubs = [
      listen('products',setProducts), listen('sales',setSales),
      listen('expenses',setExpenses), listen('orders',setOrders),
      listen('customers',setCustomers), listen('purchases',setPurchases),
      listen('debts',setDebts)
    ];
    // Listen to customer shop orders
    try {
      const shopUnsub = onSnapshot(collection(db,'shop_orders'), snap => {
        const shopOrders = snap.docs.map(d => ({id:d.id,...d.data(),fromShop:true}));
        setShopOrders(shopOrders);
      });
      unsubs.push(shopUnsub);
    } catch {}
  ...
  useEffect(()=>{
    if(!ready)return;
    saveFS('products',products);
    try{
      const threshold=settingsRef.current.lowStock||5;
      products.filter(p=>!p.transit&&(p.qty-(p.reserved||0))<=threshold&&p.qty>0)
        .forEach(p=>{
          if(!window.__qs_notified)window.__qs_notified={};
          const k=p.id+'_'+p.qty;
          if(!window.__qs_notified[k]){
            window.__qs_notified[k]=1;
            sendTelegram('⚠️ '+p.name+' заканчивается — '+( p.qty-(p.reserved||0))+' пар');
          }
        });
    }catch(e){console.warn('Low stock check:',e);}
  },[products,ready]);
  ...
  useEffect(()=>{if(ready)saveFS('returns',returns);},[returns,ready]);
  ...
  const annaShare=Math.max(0,monthProfit*0.35);

  const addSale=(productId,qty,price,note,paymentMethod,size='')=>{
    const product=products.find(p=>p.id===productId);if(!product)return;
    const sale={id:Date.now(),productId,name:product.name,qty:Number(qty),price:Number(price),cost:product.cost,total:Number(qty)*Number(price),date:nowStr(),note,paymentMethod,size,employeeId:currentEmployee?.id||null,employeeName:currentEmployee?.name||'Сотрудник'};
    setSales(prev=>[sale,...prev]);
    setProducts(prev=>prev.map(p=>p.id===productId?{...p,qty:p.qty-Number(qty)}:p));
    setReceipt(sale);
    sendTelegram('💰 Продажа! '+product.name+(size?' р.'+size:'')+' '+qty+'шт = '+fmt(Number(qty)*Number(price))+' сом');
  };
  ...
  const deleteSale=(id)=>{
    ...
    if(sale)setProducts(prev=>prev.map(p=>p.id===sale.productId?{...p,qty:p.qty+sale.qty}:p));
  ...
  const deleteReturn=(id)=>setReturns(prev=>prev.filter(r=>r.id!==id));
  const updateShopOrder=(id,status)=>{
    updateDoc(doc(db,'shop_orders',id),{status}).catch(e=>console.error('ShopOrder update:',e));
    if(status==='delivery') sendTelegram('🚚 Ваш заказ Queen Star выехал! Ожидайте доставку.');
    if(status==='done')     sendTelegram('✅ Ваш заказ Queen Star доставлен! Спасибо за покупку ❤️');
  };
  ...
  const sendTelegram=(msg)=>{
    const {telegramToken,telegramChatId}=settingsRef.current;
  ...
  const saveSettings=(newSettings)=>{
    ...
    // Save settings directly - bypass the 'same data' check
    const v = JSON.stringify(newSettings);
    saving.current['settings'] = v;
    if(fsSettings.current) fsSettings.current = newSettings;
    setDoc(doc(db,'qs','settings'),{v,ts:Date.now()}).catch(e=>console.error('Settings save:',e));
  };
  const prevShopOrdersLen = useRef(0);
  useEffect(()=>{
    if(shopOrders.length > prevShopOrdersLen.current && prevShopOrdersLen.current > 0){
      addToast('🌐 Новый заказ с сайта!', BLUE);
      sendTelegram(`🌐 <b>Заказ с сайта!</b>\n${shopOrders[0]?.customerName}\n${shopOrders[0]?.productName} р.${shopOrders[0]?.size}\n<b>${fmt(shopOrders[0]?.total)} сом</b>\n📞 ${shopOrders[0]?.customerPhone}`);
    }
    prevShopOrdersLen.current = shopOrders.length;
  },[shopOrders]);
  ...
  const addOrder=(o)=>{
    setOrders(prev=>[{id:Date.now(),date:nowStr(),status:'new',...o},...prev]);
    sendTelegram(`🛍️ <b>Новый заказ!</b>\n${o.customerName}${o.customerPhone?' ('+o.customerPhone+')':''}\n${o.productName} × ${o.qty}\n<b>${fmt(o.total)} сом</b>${o.note?'\n📍 '+o.note:''}`);
  };
  ...
  const updatePurchase=(id,field,value)=>{
    setPurchases(prev=>prev.map(p=>{
      ...
      // When purchase is marked as received → add to stock automatically
      if(field==='status'&&value==='received'){
        const existing=products.find(pr=>pr.name===p.name||pr.id===p.productId);
        if(existing){
          // Add qty to existing product
          setProducts(prev2=>prev2.map(pr=>pr.id===existing.id?{...pr,qty:pr.qty+Number(p.qty)}:pr));
          addToast('📦 '+p.name+' добавлен на склад: +'+p.qty+' пар', GREEN);
        } else {
          // Create new product on stock
          const newProduct={
            id:p.productId||'ЗК-'+Date.now().toString().slice(-4),
            name:p.name,
            qty:Number(p.qty),
            price:p.priceSom||Math.round((p.priceCny||0)*1.5*Number(settingsRef.current.cnyRate||1.5)),
            cost:Math.round((p.priceCny||0)*Number(settingsRef.current.cnyRate||1.5)),
            transit:false,
            reserved:0,
          };
          setProducts(prev2=>[...prev2,newProduct]);
          addToast('📦 Новый товар на складе: '+p.name+' +'+p.qty+' пар', GREEN);
        }
        sendTelegram('📦 Получено: '+p.name+' '+p.qty+' пар — добавлено на склад!');
      }
      return updated;
    }));
  };
  ...
  const addReturn=(r)=>{
    setReturns(prev=>[{id:Date.now(),date:nowStr(),...r},...prev]);
    setProducts(prev=>prev.map(p=>p.id===r.productId?{...p,qty:p.qty+Number(r.qty)}:p));
  };
  ...
  if(!role)return(
    <>
      ...
      <LoginScreen onLogin={(r,emp)=>{setRole(r);if(emp)setCurrentEmployee(emp);setTab('dash');}} ownerPin={settings.pin} employeePin={settings.employeePin||'1111'}/>
    </>
  );

    return(
    <div style={{background:BG,minHeight:'100vh',color:CREAM,fontFamily:"'DM Sans',sans-serif",paddingBottom:76}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,600;9..40,700&display=swap');*{box-sizing:border-box;margin:0;padding:0;}input::placeholder{color:#3A3530;}input:focus,select:focus{border-color:#C9952A!important;}select option{background:#1A1816;color:#F2EDE4;}::-webkit-scrollbar{width:3px;}::-webkit-scrollbar-thumb{background:#2A2520;}`}</style>
      <div style={{background:SURF,borderBottom:`1px solid ${BORD}`,padding:'12px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:50}}>
        ...
        <div style={{textAlign:'right'}}>
          <div style={{fontSize:11,color:role==='owner'?GOLD:MUTED,fontWeight:600}}>{role==='owner'?'👤 Амир':`🏪 ${currentEmployee?.name||'Сотрудник'}`}</div>
          <div style={{fontSize:10,marginTop:2,color:saveStatus==='ok'?GREEN:saveStatus==='saving'?GOLD:RED}}>
            {saveStatus==='ok'?'● Сохранено':saveStatus==='saving'?'⟳ Сохранение...':'⚠ Нет связи'}
          </div>
        </div>
      </div>
      ...
      <div style={S.page}>
        ...
        {tab==='kassa'&&<Kassa {...{...shared,addSale,deleteSale,todaySales,role}}/>}
        {tab==='stock'&&<Stock {...{...shared,deleteProduct,photos,onPhoto,role,settings}}/>}
        {tab==='finance'&&role==='owner'&&<Finance {...{expenses,addExpense,deleteExpense,sales,...fin}}/>}
        {tab==='calc'&&role==='owner'&&<Calc {...{settings,setSettings:saveSettings}}/>}
        {tab==='chat'&&<Chat role={role}/>}
        {tab==='more'&&role==='owner'&&<More {...{sales,expenses,...shared,...fin,settings,setSettings:saveSettings,photos,returns,addReturn,shopOrders,updateShopOrder,deleteSale,deleteOrder,deleteCustomer,deletePurchase,deleteDebt,deleteReturn,currentEmployee,onLogout:()=>{setRole(null);setCurrentEmployee(null);},orders,setOrders,customers,setCustomers,purchases,setPurchases,debts,setDebts,addOrder,addCustomer,addPurchase,addDebt,updateOrder,updateCustomer,updatePurchase,updateDebt}}/>}
      </div>
...
// ── EMPLOYEE SIMPLE LOGIN ─────────────────────────────────────
  ...
  const submit = async () => {
    ...
    const empId = name.trim().toLowerCase().replace(/\s+/g,'_');
    try {
      await setDoc(doc(db,'employees',empId), { name:name.trim(), id:empId, lastLogin:nowStr() });
    } catch {}
    onLogin('employee', { name:name.trim(), id:empId });
  };
    ...
    <div style={{width:'100%',maxWidth:300}}>
      ...
      <div style={{marginBottom:10}}>
        ...
        <input style={S.input} value={name} onChange={e=>{setName(e.target.value);setErr('');}}
          placeholder="Например: Шахло" autoFocus/>
      </div>
      <div style={{marginBottom:14}}>
        <div style={S.label}>PIN сотрудника</div>
      ...
      <div style={{fontSize:11,color:MUTED,textAlign:'center',marginTop:10}}>
        PIN задаёт владелец в настройках
      </div>
    </div>
  );
}
