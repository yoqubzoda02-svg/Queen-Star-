...
// ── HEADER ────────────────────────────────────────────────────
function Header({ cartCount, favCount, onCart, onAccount, onFav, customer }) {
  return (
...
export default function ShopApp() {
  ...
  return(
    <div style={{ background:BG, minHeight:'100vh', color:DARK, fontFamily:"'DM Sans',sans-serif", paddingBottom:20 }}>
      ...
      <Toast toasts={toasts}/>
      <Header cartCount={cart.length} favCount={favs.length} onCart={()=>setScreen('cart')} onAccount={()=>customer?setScreen('account'):setShowAuth(true)} onFav={()=>setShowFavOnly(!showFavOnly)} customer={customer}/>
